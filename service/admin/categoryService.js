...
export const toggleBlockAndUnblock = async (categoryid) => {
    try {
        ...
        if (!categoryDetail) throw new Error("Category Not Found");
...
export const editingCategory = async (categorieId) => {
    try {
        ...
        logger.error(`Error from editingCategory: ${error.message}`);
...
export const editCategoryPatchService = async (categoryName, description, fileUrl, category) => {
    try {
        let updateCategory = await categoryCollection.updateOne(
            { _id: category }, 
            { $set: { categoryName: categoryName, description: description, image: fileUrl } }
        );
        return updateCategory;
    } catch (error) {
        logger.error(`Error from editCategoryPatchService: ${error.message}`);
        throw error; // ✅ Re-throw to maintain original behavior
    }
};