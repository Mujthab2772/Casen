...
export const loginGet = (req, res) => {
  try {
    ...
    return res.render('loginPage', { loginErr, loginPassErr });
  } catch (error) {
    logger.error(`Error in loginGet: ${error.message}`);
...
export const login = async (req, res) => {
  try {
    ...
    logger.error(`Error from login: ${error.message}`);
    return res.status(STATUS_CODE.INTERNAL_SERVER_ERROR).redirect("/login");
  }
};
...
export const forgotPass = (req, res) => {
  try {
    ...
    logger.error(`Error from forgotPass: ${error.message}`);
    return res.status(STATUS_CODE.INTERNAL_SERVER_ERROR).redirect('/login');
  }
};
...
export const forgotPassResetPost = async (req, res) => {
  try {
    ...
    logger.error(`Error from forgotPassResetPost: ${error.message}`);
...
export const forgotOtpPage = async (req, res) => {
  try {
    ...
    logger.error(`Error from forgotOtpPage: ${error.message}`);
...
export const forgotOtpVerify = async (req, res) => {
  try {
    ...
    logger.error(`Error from forgotOtpVerify: ${error.message}`);
...
export const resendforgotOtp = async (req, res) => {
  try {
    req.session.otpInvalid = null;
    // Optional: log email for debugging (remove in production if sensitive)
    // logger.debug(`Resending OTP to: ${req.session.forgotEmail}`);
    ...
    logger.error(`Error from resendOtp: ${error.message}`);
    return res.status(STATUS_CODE.INTERNAL_SERVER_ERROR).redirect('/forgotOtp');
  }
};
...
export const resetPassword = (req, res) => {
  try {
    ...
    logger.error(`Error from resetPassword: ${error.message}`);
...
export const resetPasswordVerify = async (req, res) => {
  try {
    ...
    logger.error(`Error from resetPasswordVerify: ${error.message}`);
    return res.redirect('/resetPasswordPage');
  }
};